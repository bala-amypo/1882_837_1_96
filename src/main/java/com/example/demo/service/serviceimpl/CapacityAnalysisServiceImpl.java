...
public class CapacityAnalysisServiceImpl implements CapacityAnalysisService {
    ...
    public CapacityAnalysisResultDto analyzeTeamCapacity(String teamName, LocalDate start, LocalDate end) {
        ...
        for (LocalDate day : days) {
            long onLeave = leaves.stream().filter(l -> !day.isBefore(l.getStartDate()) && !day.isAfter(l.getEndDate())).count();
            double cap = ((double) (config.getTotalHeadcount() - onLeave) / config.getTotalHeadcount()) * 100;
            capacityByDate.put(day, cap);
            if (cap < config.getMinCapacityPercent()) {
                risky = true;
                alertRepo.save(new CapacityAlert(teamName, day, "HIGH", "Low capacity: " + cap + "%"));
            }
        }
